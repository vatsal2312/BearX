...
contract BearX is ERC721Enumerable, Ownable {
    ...
    // uint256 public _maxperWalletWhiteList = 1;
    // uint256 public _maxperTX = 3;
    // uint256 public _limit = 10;
    // uint256 public _reserved = 5;
    ...
    constructor(string memory baseURI, address _fundWallet) ERC721("BEARX test", "BEARX")  {
        setBaseURI(baseURI);
    ...
    function MINT_BY_TOKEN(uint256 __id1, uint256 __id2, uint256 num) public payable {
        uint256 supply = totalSupply();
        require( breedStart == true,        "Breed paused");
        require( num <= breed,              "Exceeds Breeding supply");
        require( TokenContract.balanceOf(msg.sender) >= breedPrice * num,  "Tokens sent is not correct");
        ...
        for(uint256 i; i < num; i++){
            _safeMint( msg.sender, breedIDs + i );
            breed--;
            breedIDs++;
            supply++;
        }
    ...
    function MINT(uint256 num) public payable {
        ...
        require( num <= _maxperTX,                           "You can mint a maximum of 2 BEARX" );
